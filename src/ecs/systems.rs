...
use crate::events::{Event, SHUTDOWN_REQUESTED};
use crate::manager::{
...
pub(super) fn pump_events(
    ...
    if SHUTDOWN_REQUESTED.load(std::sync::atomic::Ordering::SeqCst) {
        info!("shutdown signal observed, requesting graceful exit");
...
pub(crate) fn perform_exit_cleanup(
    all_windows: &mut Query<&mut Window>,
    displays: &Query<&Display>,
    window_manager: &WindowManager,
    overlay_mgr: Option<&mut OverlayManager>,
) {
    ...
    if let Some(overlay_mgr) = overlay_mgr {
        overlay_mgr.remove_all();
    }
    ...
    for mut window in all_windows.iter_mut() {
        let frame = window.frame();
...
pub(super) fn cleanup_on_exit(
    ...
    info!("AppExit received in cleanup_on_exit");
